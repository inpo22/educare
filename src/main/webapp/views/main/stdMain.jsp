				...
				<div>
					<div class="white-section">
						<table class="table">
							...
							<tbody>
								...
								<c:forEach items="${refBoardList}" var="post">
									<tr>
										<td><a href="/dataBoard/detail.go?post_no=${post.post_no}">${post.title}</a></td>
										<td>${post.user_name}</td>
										<td><fmt:formatDate value="${post.reg_date}" pattern="yyyy.MM.dd"/></td>
									</tr>
								</c:forEach>
						...
						<c:if test="${refBoardList.size() > 0}">
							<div class="more-div">
								<a class="blue-color" href="/dataBoard/list.go">더보기</a>
							</div>
						</c:if>
					</div>
				</div>
...
<script>
	...
	function attendance() {
		var result = confirm('출근 시간에 현재 시간을 입력하시겠습니까?');
		if (result) {
	...
	function leaveWork() {
		var result = confirm('퇴근 시간에 현재 시간을 입력하시겠습니까?');
		if (result) {
